...
String answer=request.getParameter("editor");
	...
	String sql3="SELECT on4 FROM score WHERE email ='"+session.getAttribute("email")+"'";
	 r=st.executeQuery(sql3);
while(r.next())
{	if(r.getString("on4").equals("n"))
	{
            out.print(r.getString("on4"));
...
Statement stmt=(Statement)con.createStatement();
	...
	if(answer.equals(rs.getString("ans4")))
	{
		...
		while(rs1.next())
		{
			...
			String sql1 = "update score set total='"+score+"',on4='y',lvl4=100 where email='"+session.getAttribute("email")+"'";
	...
	String sql5="SELECT on1,on2,on3,on4,on2,on4,lvl2,lvl1 FROM score WHERE email ='"+session.getAttribute("email")+"'";
	 rs2=st4.executeQuery(sql5);
		...
		response.sendRedirect(redirectURL); 
         ...
         {
                String redirectURL = "http://172.16.5.54:8080/associate/lpuzzle1.html";
		response.sendRedirect(redirectURL); 
         }
        }
	}else
