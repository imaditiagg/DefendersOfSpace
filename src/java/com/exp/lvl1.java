...
Statement stmt=(Statement)con.createStatement();
	...
	while(rs.next())
	{
            out.println(rs.getString("ans1"));
	if(answer.equals(rs.getString("ans1")))
	{
                        out.println("right ans");
			String sql1 = "update score set lvl1= 100,on1='y'  where email='"+session.getAttribute("email")+"'";
			stmt1.executeUpdate(sql1);
   		out.println("score +");
        
        }		else{		out.println("ans wrng");
                        String sql6 = "update score set lvl1= -50,on1='y' where email='"+session.getAttribute("email")+"'";
			stmt1.executeUpdate(sql6);
                        out.println("score -");
        }
        Statement st4=(Statement)con.createStatement();
	String sql5="SELECT on1,on2,on3,on4,lvl2,lvl1 FROM score WHERE email ='"+session.getAttribute("email")+"'";
